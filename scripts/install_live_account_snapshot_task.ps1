param(
    [string]$TaskName = "DummyLiveAccountSnapshot",
...
$launcher = Join-Path $repo "scripts\tasks\launch_live_account_snapshot.vbs"

if (-not (Test-Path -LiteralPath $launcher)) {
    throw "Live account snapshot launcher not found: $launcher"
}

$start = (Get-Date).AddMinutes([Math]::Max(1, $StartDelayMinutes)).ToString("HH:mm")
$cadence = [Math]::Max(5, $IntervalMinutes)
$timeoutMinutes = [Math]::Max(3, $ExecutionTimeLimitMinutes)
$arguments = "`"$launcher`""
