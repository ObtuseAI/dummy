"""Generate DUMMY v51 read-only approval surface reports."""
...
from predator_mesh.v51.reports import DEFAULT_REQUIRED_REPORT_NAMES, V51ReportFactory, VERIFICATION_COMMANDS
...
def generate_v51_report_bundle(*, env: dict[str, str] | None = None, enable_real_probe: bool | None = None, real_transport: Any | None = None, allow_live_network: bool = False) -> dict[str, dict[str, Any]]:
    env = {} if env is None else env
    exact_gate = _gate_enabled(env)
    enable_real = exact_gate if enable_real_probe is None else enable_real_probe
    return V51ReportFactory(env=env, enable_real_probe=enable_real, real_transport=real_transport, allow_live_network=allow_live_network).build()


def _build_final(reports: dict[str, dict[str, Any]], paths: dict[str, Path] | None = None) -> dict[str, Any]:
    paths = paths or {}
    mission = reports["dummy_mission_state_report_v37.json"]
    ...
    final = {
        ...
        "workstream": "v51: Operator Armed Rehearsal Gate Approval Surface And Canary Nonexecution Lock",
        "milestone": MILESTONE,
...
def generate_all_v51_reports_for_tests(**kwargs: Any) -> dict[str, dict[str, Any]]:
    reports = generate_v51_report_bundle(**kwargs)
    reports["final_report_v51.json"] = _build_final(reports)
    return reports


def _required_v51_tests() -> list[str]:
    tests: list[str] = []
    for path in sorted((ROOT / "tests").glob("test_*.py")):
        try:
            ...
            continue
        if "v51" in path.name or "predator_mesh.v51" in text or "tests.v51_test_helpers" in text:
...
def _write_final_indexes(final: dict[str, Any], final_path: Path) -> None:
    final_index = {
        ...
        "approval_surface_status": final.get("approval_surface_status"),
        "holdout_continuation_status": final.get("holdout_continuation_status"),
        "final_report_v51": str(final_path),
        "proof_paths": final.get("proof_paths", {}),
    }
    final_index["v51"] = {"generated_at": final["generated_at"], "milestone": final["milestone"], "verdict": final["verdict"], "final_report_v51": str(final_path), "partial_reason": final["partial_reason"]}
...
def main() -> dict[str, Any]:
    ...
    reports = generate_v51_report_bundle(env=env, enable_real_probe=gate, allow_live_network=gate)
    paths = {name: _write_report(name, data) for name, data in reports.items()}
    final = _build_final(reports, paths)
    final_path = _write_report("final_report_v51.json", final)
    _write_final_indexes(final, final_path)
    tests_summary = _load_report("tests_summary.json", {})
    tests_summary["v51_required_commands"] = VERIFICATION_COMMANDS
    tests_summary["v51_required_tests"] = _required_v51_tests()
    tests_summary["v51_required_reports"] = ["final_report.json", "tests_summary.json", "final_report_v51.json", *sorted(reports)]
    tests_summary["v51_report_generated_at"] = final["generated_at"]
    tests_summary["v51_final_verdict"] = final["verdict"]
    tests_summary["v51_required_test_count"] = len(tests_summary["v51_required_tests"])
    tests_summary["v51_required_report_count"] = len(tests_summary["v51_required_reports"])
