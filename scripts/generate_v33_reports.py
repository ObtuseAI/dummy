"""Generate DUMMY V33 operator-enabled public probe observation reports."""
...
from predator_mesh.v33.reports import DEFAULT_REQUIRED_REPORT_NAMES, REPORT_NAMES, V33ReportFactory
...
def generate_v33_report_bundle(*, enable_network: bool = False, env: dict[str, str] | None = None) -> dict[str, dict[str, Any]]:
    return V33ReportFactory(enable_network=enable_network, env=env or {}).build()


def _required_test_commands() -> list[str]:
    return [
        "python -m pytest tests/ -q --tb=short --timeout=60 --durations=25",
        "python -m pytest tests/test_v33_operator_enabled_probe_run_controller_v1.py tests/test_exact_gate_acknowledgement_hardening_v3.py tests/test_minimal_live_public_probe_execution_v1.py tests/test_live_public_evidence_ingestion_v3.py tests/test_settlement_evidence_join_v3.py tests/test_due_forecast_observation_run_v6.py tests/test_live_score_observation_run_v4.py tests/test_v33_required_report_manifest.py tests/test_public_probe_artifact_cache_v3.py tests/test_enabled_probe_audit_ledger_v2.py tests/test_sports_probe_exclusion_guard_v4.py tests/test_source_truth_enabled_probe_evidence_v14.py tests/test_no_missing_ack_probe_run_v33.py tests/test_no_fuzzy_ack_probe_run_v33.py tests/test_no_operator_enabled_probe_run_to_execution_bridge_v33.py tests/test_dashboard_v33.py -q --tb=short",
        "cd dashboard/frontend && npm run build",
        "python scripts/generate_v33_reports.py",
    ]


def _required_v33_tests() -> list[str]:
    tests: list[str] = []
    for path in sorted((ROOT / "tests").glob("test_*.py")):
        try:
            ...
            continue
        if "tests.v33_test_helpers" in text or "predator_mesh.v33" in text:
            tests.append(path.name)
    return tests


def _v33_report_names(reports: dict[str, dict[str, Any]] | None = None) -> list[str]:
    names = sorted((reports or generate_v33_report_bundle(enable_network=False)).keys())
    return ["final_report.json", "tests_summary.json", "final_report_v33.json", *names]
...
def _build_final(reports: dict[str, dict[str, Any]], paths: dict[str, Path] | None = None) -> dict[str, Any]:
    ...
    mission = reports["dummy_mission_state_report_v19.json"]
    ...
    final = {
        ...
        "workstream": "V33: Operator Enabled Public Probe Observation Run",
        "milestone": MILESTONE,
        ...
        "remaining_operator_actions": [
            "Set DUMMY_PUBLIC_PROBE_MODE=1 and DUMMY_PUBLIC_PROBE_ACK=READ_ONLY_PUBLIC_PROBES_ONLY before running the bounded public probe observation pass.",
    ...
    passthrough_keys = [
        ...
        "sprint_queue_v10_status",
        "compounding_v17_status",
        "next_bundle_recommendation",
        "market_class_scoreboard_v18_status",
...
def generate_all_v33_reports_for_tests(*, enable_network: bool = False) -> dict[str, dict[str, Any]]:
    reports = generate_v33_report_bundle(enable_network=enable_network)
    reports["final_report_v33.json"] = _build_final(reports)
    return reports
...
def _write_final_indexes(final: dict[str, Any], final_path: Path) -> None:
    final_index = {
        ...
        "final_report_v33": str(final_path),
...
def _write_required_report_names_manifest() -> None:
    (ARTIFACTS / "v33_required_report_names_from_attachment.txt").write_text(
...
def main() -> dict[str, Any]:
    ...
    reports = generate_v33_report_bundle(enable_network=False)
    paths = {name: _write_report(name, data) for name, data in reports.items()}
    final = _build_final(reports, paths)
    final_path = _write_report("final_report_v33.json", final)
    _write_final_indexes(final, final_path)

    tests_summary = _load_report("tests_summary.json", {})
    tests_summary["v33_required_commands"] = _required_test_commands()
    tests_summary["v33_required_tests"] = _required_v33_tests()
    tests_summary["v33_required_reports"] = _v33_report_names(reports)
    tests_summary["v33_report_generated_at"] = final["generated_at"]
    tests_summary["v33_final_verdict"] = final["verdict"]
    tests_summary["v33_required_test_count"] = len(tests_summary["v33_required_tests"])
    tests_summary["v33_required_report_count"] = len(tests_summary["v33_required_reports"])
