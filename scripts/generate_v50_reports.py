"""Generate DUMMY v50 read-only locked rehearsal preflight reports."""
...
from predator_mesh.v50.reports import DEFAULT_REQUIRED_REPORT_NAMES, V50ReportFactory, VERIFICATION_COMMANDS
...
def generate_v50_report_bundle(*, env: dict[str, str] | None = None, enable_real_probe: bool | None = None, real_transport: Any | None = None, allow_live_network: bool = False) -> dict[str, dict[str, Any]]:
    env = {} if env is None else env
    exact_gate = _gate_enabled(env)
    enable_real = exact_gate if enable_real_probe is None else enable_real_probe
    return V50ReportFactory(env=env, enable_real_probe=enable_real, real_transport=real_transport, allow_live_network=allow_live_network).build()


def _build_final(reports: dict[str, dict[str, Any]], paths: dict[str, Path] | None = None) -> dict[str, Any]:
    paths = paths or {}
    mission = reports["dummy_mission_state_report_v36.json"]
    ...
    final = {
        ...
        "workstream": "v50: Operator Armed Rehearsal Gate Locked Preflight And Nonexecution Proof",
        "milestone": MILESTONE,
    ...
    ] if final_verdict != "PASS" else ["Keep preflight locked and non-executing until explicit future operator authorization."]
    return final


def generate_all_v50_reports_for_tests(**kwargs: Any) -> dict[str, dict[str, Any]]:
    reports = generate_v50_report_bundle(**kwargs)
    reports["final_report_v50.json"] = _build_final(reports)
    return reports


def _required_v50_tests() -> list[str]:
    tests: list[str] = []
    for path in sorted((ROOT / "tests").glob("test_*.py")):
        try:
            ...
            continue
        if "v50" in path.name or "predator_mesh.v50" in text or "tests.v50_test_helpers" in text:
...
def _write_final_indexes(final: dict[str, Any], final_path: Path) -> None:
    final_index = {
        ...
        "locked_rehearsal_preflight_status": final.get("locked_rehearsal_preflight_status"),
        "stable_sample_holdout_continuation_status": final.get("stable_sample_holdout_continuation_status"),
        "final_report_v50": str(final_path),
        "proof_paths": final.get("proof_paths", {}),
    }
    final_index["v50"] = {"generated_at": final["generated_at"], "milestone": final["milestone"], "verdict": final["verdict"], "final_report_v50": str(final_path), "partial_reason": final["partial_reason"]}
...
def main() -> dict[str, Any]:
    ...
    reports = generate_v50_report_bundle(env=env, enable_real_probe=gate, allow_live_network=gate)
    paths = {name: _write_report(name, data) for name, data in reports.items()}
    final = _build_final(reports, paths)
    final_path = _write_report("final_report_v50.json", final)
    _write_final_indexes(final, final_path)
    tests_summary = _load_report("tests_summary.json", {})
    tests_summary["v50_required_commands"] = VERIFICATION_COMMANDS
    tests_summary["v50_required_tests"] = _required_v50_tests()
    tests_summary["v50_required_reports"] = ["final_report.json", "tests_summary.json", "final_report_v50.json", *sorted(reports)]
    tests_summary["v50_report_generated_at"] = final["generated_at"]
    tests_summary["v50_final_verdict"] = final["verdict"]
    tests_summary["v50_required_test_count"] = len(tests_summary["v50_required_tests"])
    tests_summary["v50_required_report_count"] = len(tests_summary["v50_required_reports"])
