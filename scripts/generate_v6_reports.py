...
def generate_blunder_separation_recheck_v4() -> dict:
    ...
    offenders: list[str] = []
    excluded = {".git", "__pycache__", ".pytest_cache", ".venv", "venv", "node_modules", "dist", "build", "tests", "scripts", "artifacts"}
...
async def generate_real_market_strategy_scan_report_v3() -> dict:
    ...
    try:
        ...
        if not scan_results and result.get("rejected_by") == "normalization":
            reason = str(result.get("reason", "")).lower()
            if "401" in reason or "403" in reason or "unauthorized" in reason or "forbidden" in reason:
...
async def generate_live_cap_firewall_rehearsal_report_v3() -> dict:
    ...
    from core import state as state_module
    from core.config_loader import load_caps
    from core.ontology import AccountMode, OrderBook, OrderBookLevel, LiveOrderRequest
    ...
    try:
        ...
        if result.get("status") in ("blocked", "no_trade", "rehearsal"):
            report["blocked_cases"].append({
                ...
                "source": result.get("rejected_by", "live_submit"),
            })
        report["autonomous_path_status"] = result.get("status")
        report["live_submitted"] = result.get("live_submitted", False)

        # Static blocker checks
        caps = load_caps()
        ...
        with patch("live_firewall.firewall.load_caps", return_value=caps):
            ...
            capped_fw = LiveBrokerFirewall(None, capped_exposure)
            # Use a small order that passes the single-order cap but violates market exposure.
            v = await capped_fw.submit_rehearsal(_req(size=1, price_cents=50), _book(), _forecast())
        ...
        report["verdict"] = "PASS" if all_blocked and not report["live_submitted"] else "FAIL"
    except Exception as exc:
        report["errors"] = [str(exc)]
        report["verdict"] = "FAIL"
    finally:
...
def generate_dashboard_v6_report() -> dict:
    ...
    return {
        ...
        "verdict": "PASS" if all(code == 200 for code in results.values() if isinstance(code, int)) and dist.exists() else "FAIL",
    }
