"""Generate DUMMY v53 explicit approval intake and quarantine manifest dry-policy reports."""
...
from predator_mesh.v53.reports import DEFAULT_REQUIRED_REPORT_NAMES, V53ReportFactory, VERIFICATION_COMMANDS
...
def _approval_from_env(env: dict[str, str]) -> dict[str, Any] | None:
    raw = env.get("DUMMY_V53_APPROVAL_INPUT_JSON")
    if not raw:
        return None
    try:
...
def generate_v53_report_bundle(*, env: dict[str, str] | None = None, enable_real_probe: bool | None = None, real_transport: Any | None = None, allow_live_network: bool = False, approval_input: dict[str, Any] | None = None) -> dict[str, dict[str, Any]]:
    ...
    return V53ReportFactory(env=env, enable_real_probe=enable_real, real_transport=real_transport, allow_live_network=allow_live_network, approval_input=approval).build()


def _build_final(reports: dict[str, dict[str, Any]], paths: dict[str, Path] | None = None) -> dict[str, Any]:
    paths = paths or {}
    mission = reports["dummy_mission_state_report_v39.json"]
    ...
    final = {
        ...
        "workstream": "v53: Explicit Rehearsal Artifact Approval Intake And Quarantine Manifest Dry Policy",
        "milestone": MILESTONE,
    ...
    final["remaining_operator_actions"] = [
        '$env:DUMMY_V53_APPROVAL_INPUT_JSON="<exact dedicated approval packet json>"',
    ...
    ] if final_verdict != "PASS" else ["Approval validated for future quarantine only; no artifacts created."]
    return final


def generate_all_v53_reports_for_tests(**kwargs: Any) -> dict[str, dict[str, Any]]:
    reports = generate_v53_report_bundle(**kwargs)
    reports["final_report_v53.json"] = _build_final(reports)
    return reports


def _required_v53_tests() -> list[str]:
    tests: list[str] = []
    for path in sorted((ROOT / "tests").glob("test_*.py")):
        try:
            ...
            continue
        if "v53" in path.name or "predator_mesh.v53" in text or "tests.v53_test_helpers" in text:
...
def _write_final_indexes(final: dict[str, Any], final_path: Path) -> None:
    final_index = {
        ...
        "approval_intake_status": final.get("approval_intake_status"),
        "quarantine_manifest_dry_policy_status": final.get("quarantine_manifest_dry_policy_status"),
        "holdout_continuation_status": final.get("holdout_continuation_status"),
        "final_report_v53": str(final_path),
        "proof_paths": final.get("proof_paths", {}),
    }
    final_index["v53"] = {"generated_at": final["generated_at"], "milestone": final["milestone"], "verdict": final["verdict"], "final_report_v53": str(final_path), "partial_reason": final["partial_reason"]}
...
def main() -> dict[str, Any]:
    ...
    reports = generate_v53_report_bundle(env=env, enable_real_probe=gate, allow_live_network=gate)
    paths = {name: _write_report(name, data) for name, data in reports.items()}
    final = _build_final(reports, paths)
    final_path = _write_report("final_report_v53.json", final)
    _write_final_indexes(final, final_path)
    tests_summary = _load_report("tests_summary.json", {})
    tests_summary["v53_required_commands"] = VERIFICATION_COMMANDS
    tests_summary["v53_required_tests"] = _required_v53_tests()
    tests_summary["v53_required_reports"] = ["final_report.json", "tests_summary.json", "final_report_v53.json", *sorted(reports)]
    tests_summary["v53_report_generated_at"] = final["generated_at"]
    tests_summary["v53_final_verdict"] = final["verdict"]
    tests_summary["v53_required_test_count"] = len(tests_summary["v53_required_tests"])
    tests_summary["v53_required_report_count"] = len(tests_summary["v53_required_reports"])
