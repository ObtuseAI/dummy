param(
    [string]$TaskName = "DummyCryptoHorizonEvidence",
    [int]$IntervalMinutes = 10,
    [int]$StartDelayMinutes = 1,
    [int]$ExecutionTimeLimitMinutes = 6
)

$ErrorActionPreference = "Stop"
$repo = "C:\src\engine\dummy"
$launcher = Join-Path $repo "scripts\tasks\launch_crypto_horizon_evidence.vbs"

if (-not (Test-Path -LiteralPath $launcher)) {
    throw "Crypto horizon evidence launcher not found: $launcher"
}

$start = (Get-Date).AddMinutes([Math]::Max(1, $StartDelayMinutes)).ToString("HH:mm")
$cadence = [Math]::Max(5, $IntervalMinutes)
$timeoutMinutes = [Math]::Max(6, $ExecutionTimeLimitMinutes)
$arguments = "`"$launcher`""
