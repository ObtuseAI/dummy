...
const endpoints = [
  ['Mission State', '/api/v32/mission-state'],
  ['Source Recovery', '/api/v32/source-recovery'],
  ['Gate', '/api/v32/gate'],
  ['Minimal Probe Pass', '/api/v32/minimal-probe-pass'],
  ['Domain Recovery', '/api/v32/domain-recovery'],
  ['Evidence', '/api/v32/evidence'],
  ['Closure', '/api/v32/closure'],
  ['Scoring', '/api/v32/scoring'],
  ['Source Truth', '/api/v32/source-truth'],
  ['Safety', '/api/v32/safety'],
];

export default function V32Dashboard() {
  ...
  const summary = useMemo(() => {
    const mission = data['Mission State']?.dummy_mission_state_report_v18 || {};
    return [
      ['Mission', mission.mission_state_verdict || mission.verdict || 'UNKNOWN'],
      ['Gate', mission.gate_state || 'UNKNOWN'],
      ['Ack', mission.ack_validation_status || 'UNKNOWN'],
      ...
      ['Safety', mission.no_source_recovery_to_execution_bridge_status || 'UNKNOWN'],
    ];
  }, [data]);

  if (loading) return <div className="p-4">Loading V32 dashboard...</div>;
  if (error) return <div className="p-4 text-red-400">Error: {error}</div>;

  return (
    <div className="p-4 space-y-5">
      <h1 className="text-2xl font-bold">Dummy V32 Source Recovery</h1>
