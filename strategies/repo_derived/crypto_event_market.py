...
class CryptoEventMarketStrategy(StrategyGenome):
    """Repo-derived crypto event market strategy.

    Converts BTC/crypto event market signals into Dummy-native TradeProposal
    objects without touching exchange order endpoints.
    """

    name = "crypto_event_market"

    def evaluate(self, forecast: Forecast, orderbook: OrderBook) -> Optional[TradeProposal]:
        """Return a TradeProposal when crypto event edge is sufficient.
        ...
        if forecast.confidence_score < Decimal("0.6"):
            return None  # no-trade: crypto signal confidence too low
        if forecast.settlement_risk_score > Decimal("0.55"):
            return None  # no-trade: elevated crypto event settlement risk
        if spread > 5:
            return None  # no-trade: spread too wide for crypto event contract
        ...
        return TradeProposal(
            ...
            risk_estimate=(
                ...
                f"settlement_risk={forecast.settlement_risk_score} crypto_confidence={forecast.confidence_score}"
            ),
            confidence_estimate=forecast.confidence_score,
            expected_fill_behavior="passive limit fill within 60s on crypto event contract",
            stop_condition="crypto event signal reverses",
            cancellation_condition="orderbook stale > 30s or event odds shift",
            cap_impact={
            ...
            compliance_verdict=ComplianceVerdict(
                passed=True,
                blocked_categories=[],
                reason="Crypto event repo-derived strategy emits TradeProposal only; no live order path",
            ),
