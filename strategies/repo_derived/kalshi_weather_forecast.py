...
class KalshiWeatherForecastStrategy(StrategyGenome):
    """Repo-derived weather forecast strategy.

    Mines NOAA/open-meteo forecasting patterns and emits a TradeProposal when
    the model probability diverges from the Kalshi market-implied probability.
    Never calls live order endpoints.
    """

    name = "kalshi_weather_forecast"

    def evaluate(self, forecast: Forecast, orderbook: OrderBook) -> Optional[TradeProposal]:
        """Return a TradeProposal when weather-model edge is sufficient.

        No-trade cases:
        - Probability delta is too small to cover fees/spread.
        - Edge after fees is non-positive.
        - Confidence is below the strategy threshold.
        - Settlement risk is elevated for the contract.
        ...
        if forecast.confidence_score < Decimal("0.6"):
            return None  # no-trade: model confidence too low
        if forecast.settlement_risk_score > Decimal("0.5"):
            return None  # no-trade: elevated weather settlement risk
        if spread > 5:
            return None  # no-trade: spread too wide for weather contract
        ...
        return TradeProposal(
            ...
            risk_estimate=(
                ...
                f"settlement_risk={forecast.settlement_risk_score} weather_model_confidence={forecast.confidence_score}"
            ),
            confidence_estimate=forecast.confidence_score,
            expected_fill_behavior="passive limit fill within 60s on weather contract",
            stop_condition="model probability realigns with market",
            cancellation_condition="orderbook stale > 30s or forecast freshness decays",
            cap_impact={
            ...
            compliance_verdict=ComplianceVerdict(
                passed=True,
                blocked_categories=[],
                reason="Weather repo-derived strategy emits TradeProposal only; no live order path",
            ),
