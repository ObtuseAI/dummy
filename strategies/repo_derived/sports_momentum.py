...
class SportsMomentumStrategy(StrategyGenome):
    """Repo-derived sports momentum/mispricing strategy.

    Mines odds/forecast logic for momentum signals and emits a TradeProposal for
    sports event contracts only. Never calls live order endpoints.
    """

    name = "sports_momentum"

    def evaluate(self, forecast: Forecast, orderbook: OrderBook) -> Optional[TradeProposal]:
        """Return a TradeProposal when sports momentum edge is sufficient.
        ...
        if abs(forecast.probability_delta) <= Decimal("0.025"):
        ...
        if forecast.confidence_score < Decimal("0.55"):
            return None  # no-trade: momentum confidence too low
        if forecast.settlement_risk_score > Decimal("0.6"):
            return None  # no-trade: elevated sports settlement risk
        if spread > 6:
            return None  # no-trade: spread too wide for sports contract
        ...
        return TradeProposal(
            ...
            risk_estimate=(
                ...
                f"settlement_risk={forecast.settlement_risk_score} momentum_confidence={forecast.confidence_score}"
            ),
            confidence_estimate=forecast.confidence_score,
            expected_fill_behavior="passive limit fill within 90s on sports event contract",
            stop_condition="momentum signal reverses or game state changes",
            cancellation_condition="orderbook stale > 30s or line moves against position",
            cap_impact={
            ...
            compliance_verdict=ComplianceVerdict(
                passed=True,
                blocked_categories=[],
                reason="Sports repo-derived strategy emits TradeProposal only; no live order path",
            ),
