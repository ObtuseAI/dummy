...
class CommoditiesEnergyStrategy(StrategyGenome):
    """Repo-derived commodities/energy price-forecast strategy.

    Applies commodities/energy price-forecast logic to Kalshi energy contracts
    and outputs TradeProposal only. Never calls live order endpoints.
    """

    name = "commodities_energy"

    def evaluate(self, forecast: Forecast, orderbook: OrderBook) -> Optional[TradeProposal]:
        """Return a TradeProposal when energy/commodity edge is sufficient.
        ...
        if forecast.confidence_score < Decimal("0.6"):
            return None  # no-trade: energy forecast confidence too low
        if forecast.settlement_risk_score > Decimal("0.5"):
            return None  # no-trade: elevated energy settlement risk
        if spread > 5:
            return None  # no-trade: spread too wide for energy contract
        ...
        return TradeProposal(
            ...
            risk_estimate=(
                ...
                f"settlement_risk={forecast.settlement_risk_score} energy_confidence={forecast.confidence_score}"
            ),
            confidence_estimate=forecast.confidence_score,
            expected_fill_behavior="passive limit fill within 120s on energy contract",
            stop_condition="energy forecast signal reverses",
            cancellation_condition="orderbook stale > 30s or energy data updates",
            cap_impact={
            ...
            compliance_verdict=ComplianceVerdict(
                passed=True,
                blocked_categories=[],
                reason="Commodities/energy repo-derived strategy emits TradeProposal only; no live order path",
            ),
