...
| `autonomy/signals/sports_intelligence.py` + `autonomy/sports/{baseball,team_scores,ufc,formula_one}.py` | Point-in-time MLB winner/total/YRFI-NRFI, NFL/NCAAF/NHL/NBA/NCAAB winner/total, UFC winner/round/distance, and F1 race-winner challengers. Models update only from completed public results and remain excluded from execution pending scope-specific forward promotion |
