...
| `autonomy/crypto_paper_twin.py` | Permanent public-read-only BTC/ETH digital twin: parallel 15-minute/hourly incumbent, recursive, and exploratory paper lanes; immutable explanations, one position per asset/expiry/lane, quote-executable taker simulation, public-print maker diagnostics, frozen epochs, paper-canary gates, and stress-only compounding proposals; independent of SHADOW/LIVE and zero broker/capital authority |
