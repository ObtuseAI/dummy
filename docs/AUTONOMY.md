...
| `autonomy/crypto_paper_twin.py` | Permanent public-read-only BTC/ETH/SOL digital twin: native 15-minute direction contracts plus hourly ladder incumbent, recursive, and exploratory paper lanes; immutable explanations, one position per asset/expiry/lane, quote-executable taker simulation, public-print maker diagnostics, frozen epochs, paper-canary gates, and stress-only compounding proposals; independent of SHADOW/LIVE and zero broker/capital authority |
