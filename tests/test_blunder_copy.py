...
DUMMY_BLUNDER = Path("C:/src/engine/dummy/core/inherited_blunder")
...
def test_blunder_source_unchanged():
    ...
    copied = {
        p.relative_to(DUMMY_BLUNDER).as_posix(): sha256_file(p)
        for p in DUMMY_BLUNDER.rglob("*") if p.is_file()
...
def test_inherited_gates_exist():
    names = {p.stem.lower() for p in DUMMY_BLUNDER.rglob("*.py")}
...
def test_manifest_matches():
    manifest = json.loads((DUMMY_BLUNDER / ".blunder_source_manifest.json").read_text())
    for rel, h in manifest.items():
        assert (DUMMY_BLUNDER / rel).exists()
        assert sha256_file(DUMMY_BLUNDER / rel) == h
