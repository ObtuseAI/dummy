...
class ApprovalSurfaceReadOnlyTransport:
    ...
    def fetch_json(self, task: Any, timeout_seconds: int) -> dict[str, Any] | list[dict[str, Any]]:
        ...
        if timeout_seconds > 12:
            raise AssertionError("V51 public observer timeout exceeded read-only budget")
        if task.source_family == "weather":
            return {"properties": {"temperature": {"value": 17.0 + task.cycle}, "timestamp": f"2026-07-05T6{task.cycle}:{task.request_index}0:00Z"}}
        if task.source_family == "crypto":
            return {"data": {"amount": str(72000 + task.cycle * 100 + task.request_index)}, "timestamp": f"2026-07-05T6{task.cycle}:{task.request_index}5:00Z"}
        if task.source_family == "public_event_reference":
            return [{"indicator": {"id": "FP.CPI.TOTL.ZG"}, "value": 2.0 + task.request_index, "date": "2025"}]
        raise AssertionError(f"unexpected source family: {task.source_family}")


def v51_reports(**kwargs: Any) -> dict[str, dict[str, Any]]:
    from archive.report_scripts.generate_v51_reports import generate_all_v51_reports_for_tests

    return generate_all_v51_reports_for_tests(**kwargs)


def v51_enabled_reports() -> dict[str, dict[str, Any]]:
    from predator_mesh.v36.run import EXACT_GATE_ENV

    return v51_reports(env=EXACT_GATE_ENV, enable_real_probe=True, real_transport=ApprovalSurfaceReadOnlyTransport())


def assert_v51_safe(report: dict[str, Any]) -> None:
    ...
    assert report["approval_surface_policy_only"] is True
    assert report["v51_execution_artifacts_created"] is False


def assert_v51_report_named(name: str, key: str | None = None, *, enabled: bool = False) -> dict[str, Any]:
    reports = v51_enabled_reports() if enabled else v51_reports()
    assert name in reports, f"missing report: {name}"
    report = reports[name]
    assert_v51_safe(report)
    if key is not None:
