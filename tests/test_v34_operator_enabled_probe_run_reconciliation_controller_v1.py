...
from predator_mesh.v34.run import V34OperatorEnabledProbeRunReconciliationControllerV1, build_default_v34_state
from tests.v34_test_helpers import assert_v34_report_named


def test_v34_controller_preserves_default_disabled_operator_action() -> None:
    state = build_default_v34_state(enable_network=False)
    result = V34OperatorEnabledProbeRunReconciliationControllerV1().run(state)
...
def test_v34_controller_enabled_fake_path_runs_bounded_reconciliation_pass() -> None:
...
def test_v34_controller_report_contract() -> None:
