...
class RehearsalGateReviewReadOnlyTransport:
    ...
    def fetch_json(self, task: Any, timeout_seconds: int) -> dict[str, Any] | list[dict[str, Any]]:
        ...
        if timeout_seconds > 12:
            raise AssertionError("V49 public observer timeout exceeded read-only budget")
        if task.source_family == "weather":
            return {"properties": {"temperature": {"value": 20.0 + task.cycle}, "timestamp": f"2026-07-05T4{task.cycle}:{task.request_index}0:00Z"}}
        if task.source_family == "crypto":
            return {"data": {"amount": str(70000 + task.cycle * 100 + task.request_index)}, "timestamp": f"2026-07-05T4{task.cycle}:{task.request_index}5:00Z"}
...
def v49_enabled_reports() -> dict[str, dict[str, Any]]:
    from predator_mesh.v36.run import EXACT_GATE_ENV

    return v49_reports(env=EXACT_GATE_ENV, enable_real_probe=True, real_transport=RehearsalGateReviewReadOnlyTransport())


def assert_v49_safe(report: dict[str, Any]) -> None:
...
def assert_v49_report_named(name: str, key: str | None = None, *, enabled: bool = False) -> dict[str, Any]:
    reports = v49_enabled_reports() if enabled else v49_reports()
    assert name in reports, f"missing report: {name}"
    report = reports[name]
    assert_v49_safe(report)
    if key is not None:
