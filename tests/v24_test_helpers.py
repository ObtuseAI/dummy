...
_EXPLICIT_REPORTS = {
    "dummy_mission_state_v24": "dummy_mission_state_report_v10.json",
    "dashboard_v24": "dashboard_v24_report_v1.json",
    "timeout_guards_still_intact_v24": "v24_runtime_budget_report_v1.json",
    "v17_truth_loop_still_passes_v24": "final_report_v24.json",
    "v21_source_activation_still_passes_v24": "final_report_v24.json",
    "v22_forecast_write_breakthrough_still_passes_v24": "final_report_v24.json",
    "v23_observer_calibration_still_passes_or_partial_expected_v24": "final_report_v24.json",
}
...
def _candidate_report_names(stem: str) -> list[str]:
    ...
    if stem.endswith("_v24") and (
        stem.startswith("no_")
        or stem.startswith("readonly_only_")
        or stem in {"blunder_separation_v24", "dummy_canonical_identity_v24"}
    ):
        base = stem.removesuffix("_v24")
        if base == "blunder_separation":
            return ["blunder_separation_recheck_v24.json"]
        if base == "dummy_canonical_identity":
            return ["dummy_canonical_identity_report_v24.json"]
        return [f"{base}_report_v24.json"]
    return [
...
def report_name_for_test(test_file: str | Path) -> str:
    ...
    for candidate in _candidate_report_names(stem):
        if candidate in reports:
            return candidate
    raise AssertionError(f"No V24 report mapped for {stem}")
...
def assert_v24_report_named(report_name: str, *required_keys: str) -> dict:
    reports = v24_reports()
...
def assert_current_test_report(test_file: str | Path) -> dict:
    return assert_v24_report_named(report_name_for_test(test_file), "workstream")
