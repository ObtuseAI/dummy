...
class LockedPreflightReadOnlyTransport:
    ...
    def fetch_json(self, task: Any, timeout_seconds: int) -> dict[str, Any] | list[dict[str, Any]]:
        ...
        if timeout_seconds > 12:
            raise AssertionError("V50 public observer timeout exceeded read-only budget")
        if task.source_family == "weather":
            return {"properties": {"temperature": {"value": 18.0 + task.cycle}, "timestamp": f"2026-07-05T5{task.cycle}:{task.request_index}0:00Z"}}
        if task.source_family == "crypto":
            return {"data": {"amount": str(71000 + task.cycle * 100 + task.request_index)}, "timestamp": f"2026-07-05T5{task.cycle}:{task.request_index}5:00Z"}
...
def v50_enabled_reports() -> dict[str, dict[str, Any]]:
    from predator_mesh.v36.run import EXACT_GATE_ENV

    return v50_reports(env=EXACT_GATE_ENV, enable_real_probe=True, real_transport=LockedPreflightReadOnlyTransport())


def assert_v50_safe(report: dict[str, Any]) -> None:
    ...
    assert report["locked_preflight_design_only"] is True
    assert report["v50_execution_artifacts_created"] is False


def assert_v50_report_named(name: str, key: str | None = None, *, enabled: bool = False) -> dict[str, Any]:
    reports = v50_enabled_reports() if enabled else v50_reports()
    assert name in reports, f"missing report: {name}"
    report = reports[name]
    assert_v50_safe(report)
    if key is not None:
