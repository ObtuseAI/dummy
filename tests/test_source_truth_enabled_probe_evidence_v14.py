...
from predator_mesh.v33.run import SourceTruthEnabledProbeEvidenceV14, build_default_v33_state
from tests.v33_test_helpers import assert_current_test_report


def test_source_truth_enabled_probe_evidence_default_partial_action() -> None:
    truth = SourceTruthEnabledProbeEvidenceV14().evaluate(build_default_v33_state(enable_network=False))
...
def test_source_truth_enabled_probe_evidence_report_contract() -> None:
    report = assert_current_test_report(__file__)

    assert report["source_truth_enabled_probe_evidence_v14_status"] == "PASS_WITH_REMAINING_PARTIALS"
    assert report["enabled_source_recovery_action_v14"] == "operator must set exact read-only public probe gate"
